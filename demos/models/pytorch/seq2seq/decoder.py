...
from demos.models.pytorch.seq2seq.attention import DotProductAttention
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.rnn += [nn.LSTMCell(input_size=self.embedding_dim + self.encoder_hidden_size,
                                 hidden_size=self.hidden_size)]
        for layer in range(1, self.num_layers):
            self.rnn += [nn.LSTMCell(input_size=self.hidden_size,
                                     hidden_size=self.hidden_size)]
        self.attention = DotProductAttention()
        self.mlp = nn.Sequential(
            nn.Linear(in_features=self.encoder_hidden_size + self.hidden_size,
    ...
    @staticmethod
    def zero_state(encoder_padded_outputs, hidden_size, H=None):
        N = encoder_padded_outputs.size(0)
        H = hidden_size if H is None else H

        return encoder_padded_outputs.new_zeros(N, H)

    def forward(self, padded_input, encoder_padded_outputs):
        """
        :param padded_input: N * To
        # encoder_hidden: (num_layers * num_directions) x N x H
        :param encoder_padded_outputs: N x Ti x H
        :return:
        """
        ...
        # TODO: should minus 1(sos)?
        # max_length = ys_in_pad.size(1) - 1

        # ############## Init decoder rnn
        h_list = [Decoder.zero_state(encoder_padded_outputs=encoder_padded_outputs,
                                     hidden_size=self.hidden_size)]
        c_list = [Decoder.zero_state(encoder_padded_outputs=encoder_padded_outputs,
                                     hidden_size=self.hidden_size)]
        for layer in range(1, self.num_layers):
            h_list.append(Decoder.zero_state(encoder_padded_outputs=encoder_padded_outputs,
                                             hidden_size=self.hidden_size))
            c_list.append(Decoder.zero_state(encoder_padded_outputs=encoder_padded_outputs,
                                             hidden_size=self.hidden_size))
        att_c = self.zero_state(encoder_padded_outputs=encoder_padded_outputs,
                                hidden_size=self.hidden_size,
