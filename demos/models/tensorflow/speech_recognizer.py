...
from demos.utils.model_util import Util
...
class SpeechRecognizer:
    ...
    def infer(self, inputs, restore_path, targets=None):
        ...
        if targets is not None:
            ...
            for (inps, trgts) in Util.minibatches(inputs,
                                                  targets=targets,
            ...
            for inps in Util.minibatches(inputs,
                                         targets=None,
                                         minibatch_size=self.batch_size):
    ...
    def run_epoch(self, inputs, targets, epoch):
        ...
        for i, (inps, trgts) in enumerate(Util.minibatches(inputs,
                                                           targets,
                                                           self.batch_size)):
    ...
    def run_evaluate(self, inputs, targets, epoch):
        ...
        if self.summary_dir is not None:
            eval_losses = []
            for inps, trgts in Util.minibatches(inputs, targets, self.batch_size):
    ...
    def get_feed_dict(self, inps, trgts=None):
        ...
        if self.mode != 'INFER':
            ...
            inp_ids, audio_sequence_lengths = Util.pad_audio_sequences(inps)
            feed = {
                self.audios: inp_ids,
            ...
            if trgts is not None:
                trgt_ids, char_sequence_lengths = Util.pad_txt_sequences(
                    trgts,
            ...
            if trgts is not None:
                _, char_sequence_lengths = Util.pad_txt_sequences(
                    trgts,
                    self.char2ind[self.pad],
                )
