...
class AM(object):
    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            self.m_model.fit_generator(generator=train_dev_generator, steps_per_epoch=self.args.save_step)
            ...
            if (epoch + 1) % 50 == 0:
                ...
                self.a_model.save(filepath=self.args.AM_DIR.joinpath(self.nn_type + '_' + str(epoch) + '.h5'))
                self.m_model.save(
                    filepath=self.args.AM_DIR.joinpath(self.nn_type + '_' + 'ctc_' + str(epoch) + '.h5'))
